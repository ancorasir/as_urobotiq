...
def inputs(filenames, batch_size, num_epochs):
    ...
    with tf.name_scope('input'):
        ...
        images, labels = read_and_decode_single_example(filename_queue)
        ...
        images_batch, labels_batch = tf.train.shuffle_batch([images, labels], batch_size=batch_size, capacity=1000 + 3 * batch_size, min_after_dequeue=1000)
        actual_batch_size = images_batch.get_shape()[0].value*images_batch.get_shape()[1].value
        return tf.reshape(images_batch, [actual_batch_size, 472*2, 472, 3]), tf.reshape(labels_batch, [actual_batch_size, 1])
